...
int wmain() {
    ...
    while (true) {
        ...
        // Variant: simplified screenshot formula only
        ...
        for (auto& t : tempRows) {
            ...
            r.whiteWorker = bySqrtCycles;
            r.confirmedByMaxNorm = false;
            r.confirmedByDutyCycle = bySqrtCycles;
            r.score = scoreSqrtCycles;
        ...
        for (const auto& r : rows) {
            log
                ...
                << L" ConfirmedBy="
                << (r.whiteWorker ? (r.confirmedByMaxNorm && r.confirmedByDutyCycle ? L"SQRT_CYCLES") : L"-")
